...
KILLCHAIN_STAGE_DELAY = 480.0  # 8 minutes between stages — accounts for Azure Event Hub lag
...
def prepare_payload(raw: dict, log_type: str, now_iso: str) -> dict:
    ...
    # event.time MUST be current Unix epoch ms — 0 or None triggers injection
    ...
    if log_type.startswith("Okta"):
        ...
        # Inject current timestamp — old published values skip rule evaluation
    ...
    # Google Reports API nested format — events array required
    if log_type.startswith("GSuite"):
        ...
        # Move flat name/type/parameters into nested events array
        evt_name   = raw.pop("name", "")
        evt_type   = raw.pop("type", "login")
        evt_params = raw.pop("parameters", {})
        # Convert flat params dict to Google array format
        params_list = [{"name": k, "value": v} for k, v in evt_params.items()]                       if isinstance(evt_params, dict) else evt_params
        raw["events"] = [{
            "type":       evt_type,
            "name":       evt_name,
...
def send_killchain(events: list[dict], dry_run: bool, delay: float) -> tuple[int, int]:
    ...
    stage_labels = {
        "killchain_stage_1": "Stage 1 — SentinelOne LOLBin / Credential Access",
        "killchain_stage_2": "Stage 2 — Entra ID MFA Reset / Admin Role",
        "killchain_stage_3": "Stage 3 — AWS Backup Destruction",
    }
...
def main():
    ...
    parser.add_argument("--tenant",         default="beazley",
                        help="Target tenant from tenants.yml (default: beazley)")
    parser.add_argument("--malicious-only", action="store_true")
    parser.add_argument("--benign-only",    action="store_true")
    parser.add_argument("--vendor",         help="crowdstrike | sentinelone | carbonblack | defender | azure_audit | aws_cloudtrail")
    ...
    if args.killchain:
        ...
        print(f"{'='*60}")
        print(f"  Stages       : S1 LOLBin → Entra Identity Takeover → AWS Backup Destruction")
        print(f"  Stage delay  : {int(KILLCHAIN_STAGE_DELAY)}s ({int(KILLCHAIN_STAGE_DELAY/60)} min) — accounts for Azure Event Hub lag")
